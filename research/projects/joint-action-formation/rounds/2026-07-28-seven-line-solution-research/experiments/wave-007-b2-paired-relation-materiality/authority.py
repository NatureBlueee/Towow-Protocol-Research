"""Parent-process authority broker for Wave 007-B2.

The candidate is a spawned JSON-RPC worker.  No broker reference, private
world state, runtime private key, evidence inventory, or mutable audit handle
...
def build_contract(
    runtime_signers: dict[str, Ed25519PrivateKey],
) -> dict[str, Any]:
    return {
...
class AuthorityBroker:
    def __init__(
        ...
        *,
        ...
        elif evaluator_attack == "DUPLICATE_AUTHORIZATION":
            self.__evidence["auth_offer"] = copy.deepcopy(
                self.__evidence["auth_seek"]
            )
        elif evaluator_attack == "CHANGED_PROPOSAL_BYTES":
            self.__evidence["proposal"]["evidence"]["body"][
                "relation_version"
            ] = 99
    ...
    def runner_bootstrap(self) -> dict[str, Any]:
        """Return only candidate-safe process bootstrap data."""
        return {"opaque_handle": self.__opaque_handle}

    def snapshot(self) -> BrokerSnapshot:
        """Return copies; no mutable authority-owned collection escapes."""
        return BrokerSnapshot(
            contract=copy.deepcopy(self.__contract),
            operation_log=copy.deepcopy(self.__operation_log),
            evidence_returns=copy.deepcopy(self.__returns),
        )

    def dispatch(
        self, method: str, params: dict[str, Any]
    ) -> dict[str, Any] | None:
        """Dispatch the fixed RPC allowlist in the parent process."""
        handlers = {
            "read_evidence": lambda: self.__read_evidence(
                params["name"]
            ),
            "verify_evidence": lambda: self.__verify_evidence(
                params["evidence"]
            ),
            "record_relation_decision": lambda: (
                self.__record_relation_decision(
                    params["state"], params["evidence_refs"]
                )
            ),
            "request_reuse": lambda: self.__request_reuse(
                params["authorizations"]
            ),
            "poll_withdrawal": self.__poll_withdrawal,
        }
            ...
            self.__operation_log.append(
                {
                    "op": "RPC_REJECTED",
                    "method": method,
                    "bytes": len(canonical_bytes(params)),
                    "disclosure_units": 0,
                }
            )
    ...
    def __read_evidence(self, name: str) -> dict[str, Any]:
        if name in self.__deleted_evidence:
            returned = self.__observation(
                "UNKNOWN",
                issuer="CONTROLLER-W7B",
                category=name,
        ...
        self.__returns[name] = copy.deepcopy(returned)
        return returned

    def __verify_evidence(
        ...
        self.__operation_log.append(
            {
                "op": "VERIFY_SIGNATURE",
                "evidence_ref": (
                    envelope_hash(evidence) if evidence else None
                ),
    ...
    def __record_relation_decision(
    ...
    def __request_reuse(
        ...
        for item in authorizations:
            ...
            if (
                ...
                proposal_hashes.add(body["proposal_sha256"])
                relation_ids.add(body["relation_id"])
                relation_versions.add(body["relation_version"])
        authorized = (
            self.__private_state.reuse_truth
            == "EXPLICIT_BOUNDED_REUSE_AUTHORIZED"
            and self.__private_state.relation_evidence
            == "VALID_BOUNDED_REUSE"
            ...
            and self.__reuse_count == 0
        )
        if authorized:
            self.__reuse_count += 1
            response = self.__sign(
                "CONTROLLER-W7B",
                "REUSE_EXECUTION_RECEIPT",
                {
                    **self.__base(),
                    "reuse_operation": REUSE_OPERATION,
                    "reuse_count": self.__reuse_count,
        ...
        self.__returns[
            f"reuse_response_{self.__reuse_count}"
        ] = copy.deepcopy(returned)
        return returned

    def __poll_withdrawal(self) -> dict[str, Any]:
        self.__operation_log.append(
            {
                "op": "POLL_WITHDRAWAL",
                "after_reuse_count": self.__reuse_count,
                "bytes": 0,
                "disclosure_units": 0,
            }
        )
        if (
            self.__private_state.withdraw_after_first_reuse
            and self.__reuse_count >= 1
        ):
            returned = copy.deepcopy(self.__evidence["withdrawal"])
        elif self.__private_state.reuse_truth == "ONE_OPERATION_ONLY":
            returned = copy.deepcopy(self.__evidence["withdrawal"])
        else:
            returned = self.__observation(
                "UNKNOWN",
        ...
        self.__returns["withdrawal_poll"] = copy.deepcopy(returned)
        return returned
