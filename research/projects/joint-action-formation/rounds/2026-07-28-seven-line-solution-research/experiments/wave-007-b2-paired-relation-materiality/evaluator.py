...
def reconstruct_implementation_identity(
    ...
    observed_interface = [
        ...
        if item["op"] == "READ_EVIDENCE"
    ]
    if any(
        item["op"] == "POLL_WITHDRAWAL"
        for item in run["operation_log"]
    ):
        observed_interface.append("withdrawal")
    registry = load_json("representations.json")["representations"]
    matches = [
        item["representation_id"]
        for item in registry
        if item["visible_evidence"] == observed_interface
    ]
    implementation_id = (
        matches[0] if len(matches) == 1 else "UNRESOLVED_IMPLEMENTATION"
    )
    return {
        "implementation_id": implementation_id,
        "observed_interface": observed_interface,
        "runner_binding_valid": (
            run["runner"]["implementation_id"] == implementation_id
        ),
    }
...
def evaluate_run(run: dict[str, Any]) -> dict[str, Any]:
    ...
    identity = reconstruct_implementation_identity(run)
    ...
    return {
        ...
        "candidate_claimed_label_ignored": run["candidate_output"][
            "candidate_label"
        ],
        "identity_source": "RAW_OPERATIONS_MATCHED_TO_RUNNER_REGISTRY",
        "runner_identity_binding_valid": identity[
            "runner_binding_valid"
        ],
...
def evaluate() -> dict[str, Any]:
    ...
    bounded_relation = next(
        ...
        == "PW-BOUNDED-VALID"
        and item["evaluation"]["implementation_id"]
        == "BOUNDED_RELATION"
    )
    ...
    runner_identity_tamper_eval = evaluate_run(
        mutations["runner_identity_tamper"]
    )
    ...
    rpc_probe_eval = evaluate_run(mutations["rpc_boundary_probe"])

    by_world_rep = {
        (
            item["evaluation"]["evaluator_world_id"],
            item["evaluation"]["implementation_id"],
        ): item["evaluation"]
        for item in baseline
    }
    one_shot_task_bound = [
    ...
    return {
        ...
        "mutation_results": {
            ...
            "runner_identity_ignores_candidate_label": (
                label_swap_eval["implementation_id"]
                == "BOUNDED_RELATION"
                and label_swap_eval[
                    "candidate_claimed_label_ignored"
                ]
                == "NO_EVIDENCE"
            ),
            "runner_identity_field_tamper_rejected": (
                runner_identity_tamper_eval["implementation_id"]
                == "BOUNDED_RELATION"
                and not runner_identity_tamper_eval[
                    "runner_identity_binding_valid"
                ]
            ),
            ...
            "candidate_process_is_separate": (
                mutations["rpc_boundary_probe"]["runner"]["parent_pid"]
                != mutations["rpc_boundary_probe"]["runner"][
                    "worker_pid"
                ]
                and not mutations["rpc_boundary_probe"]["runner"][
                    "broker_object_transferred"
                ]
                and not mutations["rpc_boundary_probe"]["runner"][
                    "audit_handle_transferred"
                ]
            ),
            "forbidden_rpc_methods_rejected": (
                set(
                    mutations["rpc_boundary_probe"][
                ...
                and all(
                    ...
                    for value in mutations["rpc_boundary_probe"][
                        "forbidden_rpc_probe_results"
                    ].values()
                )
            ),
            "forbidden_log_clear_did_not_clear_parent_log": (
                sum(
                    item["op"] == "RPC_REJECTED"
                    for item in mutations["rpc_boundary_probe"][
                        "operation_log"
                    ]
                )
                == 3
                and rpc_probe_eval["reuse_executed"]
                and rpc_probe_eval["relation_constituted"]
            ),
            "runtime_public_keys_differ_across_runs": (
                {
                    row["issuer"]: row["public_key_hex"]
                    for row in base_run["contract"][
                        "verification_keys"
                    ]
                }
                != {
                    row["issuer"]: row["public_key_hex"]
                    for row in mutations["opaque_rename"]["contract"][
                        "verification_keys"
                    ]
                }
            ),
        ...
        "overall_grade": None,
        "repair_status": "SAME_RESEARCHER_REPAIR_NOT_INDEPENDENT_EVIDENCE",
        "evidence_boundary": (
            "LOCAL_SYNTHETIC_PROCESS_ISOLATION_SELF_TEST_"
            "PENDING_ROOT_REAUDIT"
        ),
    }
