#!/usr/bin/env python3
"""Build physically separated method-visible packets from controller input.

The builder performs routing only. It does not inspect oracle truth, infer
opportunities, choose states, or produce a candidate submission.
"""
...
def build_packets(source: dict[str, Any]) -> dict[str, dict[str, Any]]:
    """Return deterministic path->packet mappings without writing files."""
...
def packet_index(packets: dict[str, dict[str, Any]]) -> dict[str, Any]:
    ...
    return {
        "schema_version": "1.0",
        "kind": "METHOD_VISIBLE_PACKET_INDEX",
        "packet_count": len(entries),
        "packets": entries,
        "delivery_rule": (
            "Deliver exactly one recipient packet to each execution domain; "
            "never deliver the index as a substitute for scoped packets."
        )
    }
...
def write_packets(
    ...
    (output_dir / "index.json").write_bytes(canonical_bytes(index))
    return index
