#!/usr/bin/env python3
"""Candidate-side strategies; the object is a JSON/RPC proxy, not the broker."""
...
class EvidenceProxy(Protocol):
    def get_request_context(self) -> dict[str, Any]: ...
    def read(
        self, evidence_type: str, *, retry: bool = False
    ) -> dict[str, Any]: ...
...
def declaration(api: EvidenceProxy) -> dict[str, Any]:
    context = api.get_request_context()
    response = api.read("declaration")
    epistemic = _epistemic_response(api, response, context)
    if epistemic:
        return {**epistemic, "implementation_id": "IMPL_DECLARATION"}
...
def latest_probe(api: EvidenceProxy) -> dict[str, Any]:
    context = api.get_request_context()
    response = api.read("probe")
    epistemic = _epistemic_response(api, response, context)
    if epistemic:
        return {**epistemic, "implementation_id": "IMPL_LATEST_PROBE"}
    record = response["record"]
...
def receipt_window(api: EvidenceProxy) -> dict[str, Any]:
    context = api.get_request_context()
    response = api.read("receipt_history")
    epistemic = _epistemic_response(api, response, context)
    if epistemic:
        return {**epistemic, "implementation_id": "IMPL_RECEIPT_WINDOW"}
    records = response.get("records", [])
    record_ids = [record.get("record_id") for record in records]
    if (
        len(records) != 3
        or any(not isinstance(record_id, str) for record_id in record_ids)
        or len(set(record_ids)) != 3
    ):
        return {
            "implementation_id": "IMPL_RECEIPT_WINDOW",
            "rely": False,
            "decision_state": "UNKNOWN",
            "reason": "receipt_window_incomplete_or_not_unique",
        }
...
def sla_recovery(api: EvidenceProxy) -> dict[str, Any]:
    context = api.get_request_context()
    sla_response = api.read("sla")
    epistemic = _epistemic_response(api, sla_response, context)
    if epistemic:
        return {**epistemic, "implementation_id": "IMPL_SLA_RECOVERY"}
    ...
    rely = bool(
        payload.get("max_latency_ms", context["deadline_ms"] + 1)
        <= context["deadline_ms"]
...
STRATEGIES: dict[str, Callable[[EvidenceProxy], dict[str, Any]]] = {
