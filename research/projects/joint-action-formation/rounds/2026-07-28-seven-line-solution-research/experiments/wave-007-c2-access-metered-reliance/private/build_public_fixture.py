#!/usr/bin/env python3
"""Build C2 evidence with authority status bound to a trusted current head."""
...
def private_key(issuer: str) -> Ed25519PrivateKey:
    seed = hashlib.sha256(
        f"wave007c2::{issuer}".encode("utf-8")
    ).digest()
...
def context(
    ...
    *,
    ...
    authority_epoch = step
    authority_head = hashlib.sha256(
        (
            f"{token}|{step}|{key_id}|{authority_status}|"
    ...
    return {
        ...
        "authority_contract_version": "AUTHORITY-CONTRACT-v2",
    }
    ...
    *,
    ...
    status_record = signed_record(
        ...
        "authority_status",
        {
            ...
            "key_id": key_id,
            "issued_step": step,
            "authority_head": ctx["authority_head"],
            "authority_epoch": ctx["authority_epoch"],
            "authority_contract_version": (
                ctx["authority_contract_version"]
            ),
        },
        0.1,
    )
...
def build() -> dict[str, Any]:
    worlds: dict[str, Any] = {
        "w7c-8f13a0": valid_world("w7c-8f13a0"),
        "w7c-50de71": valid_world("w7c-50de71"),
        "w7c-a76e20": valid_world(
            "w7c-a76e20",
            step=8,
            authority_status="REVOKED",
            health="RED",
        ),
        "w7c-3d9b14": valid_world("w7c-3d9b14"),
        "w7c-e021f8": valid_world(
            "w7c-e021f8",
            request_environment="SIM-ENV-B",
            evidence_environment="SIM-ENV-A",
        ),
        "w7c-c6430e": valid_world("w7c-c6430e"),
        "w7c-926db1": valid_world(
            "w7c-926db1",
            step=9,
        ...
        "w7c-4ef270": valid_world(
            "w7c-4ef270",
            step=9,
            key_id="RECIPIENT-KEY-v2",
            health="RECOVERED",
        ),
        "w7c-f010aa": valid_world(
            "w7c-f010aa",
            probe_result="FAILURE",
            health="RED",
        ),
        "w7c-332faa": valid_world(
            "w7c-332faa",
            probe_result="FAILURE",
        ),
        "w7c-610cdd": valid_world(
            "w7c-610cdd",
            receipt_count=1,
        ),
        "w7c-d2049e": valid_world(
            "w7c-d2049e",
            sla_status="EXPIRED",
        ),
        "w7c-b1856c": {
