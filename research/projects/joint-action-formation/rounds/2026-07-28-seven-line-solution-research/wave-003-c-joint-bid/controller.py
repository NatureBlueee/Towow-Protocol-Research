#!/usr/bin/env python3
"""Deterministic disclosure controller for the T4 joint-bid blind task.

The controller is evaluator-side infrastructure, not a candidate method. It
returns only the frozen disclosure for an allowed Authority/request pair,
enforces declared prerequisites for probes and reservations, and maintains a
hash-chained run history.
"""
...
BEHAVIOR_VERSION = "T4-DISCLOSURE-CONTROLLER-V1"
...
MAX_QUERIES_PER_ROUND = 40
MAX_FIELD_LENGTH = 2048
ALLOWED_RETENTION_SCOPES = {"EPHEMERAL", "RUN_ONLY"}


class ControllerError(ValueError):
    """Safe controller error whose text contains no hidden truth."""
...
def validate_batch(batch: dict[str, Any]) -> None:
    ...
    if (
        ...
        raise ControllerError("round must be a positive integer")
...
def pair_key(authority_id: str, request_type: str) -> str:
    return f"{authority_id}|{request_type}"
...
def build_indexes(
    ...
    valid_pair_keys = {pair_key(*item) for item in allowed_pairs}
    for disclosure in disclosures.values():
        if any(item not in valid_pair_keys for item in disclosure["prerequisites"]):
            raise ControllerError("controller prerequisite closure is invalid")
...
def response_for_query(
    ...
    key = (authority_id, request_type)
    if key not in allowed_pairs:
        return "REFUSE", "REFUSE", {}, "REQUEST_NOT_ALLOWED_FOR_AUTHORITY"
    disclosure = disclosures[key]
    unmet = [
        item for item in disclosure["prerequisites"] if item not in disclosed_pairs
    ]
    if unmet:
        return "DEFER", "DEFER", {}, "PREREQUISITES_NOT_MET"
    return (
        "DISCLOSE",
...
def process_batch(
    ...
    if (
        ...
        or not isinstance(run.get("disclosed_pairs"), list)
    ):
    ...
    disclosed_pairs = set(run["disclosed_pairs"])
    ...
    for index, raw_query in enumerate(batch["queries"]):
        ...
        request_fingerprint = digest({"task_id": TASK_ID, "query": query})
        ...
        if decision == "DISCLOSE":
            disclosed_pairs.add(
                pair_key(query["authority_id"], query["request_type"])
            )

    run["disclosed_pairs"] = sorted(disclosed_pairs)
    response_base = {
        "schema_version": "1.0",
        "task_id": TASK_ID,
        "controller_behavior_version": BEHAVIOR_VERSION,
    ...
    history_hash = digest(
        {"previous_history_hash": prior_history_hash, "round_hash": round_hash}
    )
    output = {
        **response_base,
    ...
    existing_rounds.append(
        {"round": round_number, "input_hash": input_hash, "output": output}
    )
    return output
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Run one deterministic T4 disclosure round."
    )
    parser.add_argument(
        "--input",
        required=True,
