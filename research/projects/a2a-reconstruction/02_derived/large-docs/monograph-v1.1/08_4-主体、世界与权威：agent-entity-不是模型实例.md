...
\[
E_{id}=\langle id, roots\rangle,
\qquad E_{authority}=\langle loci, mandates, policy\rangle,
\]

\[
E_{execution}=\langle executors, resources\rangle,
\qquad E_{state}=\langle memory, provenance\rangle.
\]
