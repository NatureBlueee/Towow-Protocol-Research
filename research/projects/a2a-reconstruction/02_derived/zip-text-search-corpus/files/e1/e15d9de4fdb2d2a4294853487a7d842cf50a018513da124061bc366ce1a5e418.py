...
class CaseStore:
    ...
    def revoke_mandate(self, party_id: str, mandate_id: str, *, actor: str, reason: str) -> Path:
        path = self.root / "private" / party_id / f"{mandate_id}.json"
        if not path.exists():
            raise FileNotFoundError(path)
        mandate = read_json(path)
        if mandate.get("status") == "REVOKED":
            raise ValueError(f"Mandate already revoked: {mandate_id}")
        mandate["status"] = "REVOKED"
        mandate["revoked_at"] = datetime.now(timezone.utc).isoformat()
        mandate["revoked_by"] = actor
        mandate["revocation_reason"] = reason
        write_json(path, mandate)
        self.append_event(
            event_type="MANDATE_REVOKED", actor=actor, authority_ref=mandate_id,
            relation_version=self.case().get("current_relation_version"),
            payload={"party_id": party_id, "mandate_id": mandate_id, "reason": reason},
        )
        return path

    def record_stance(
        self, *, party_id: str, stance: str, actor: str, authority_ref: str,
        relation_version: int | None = None, notes: str | None = None,
    ) -> dict[str, Any]:
        case = self.case()
        if party_id not in case.get("parties", {}):
            raise ValueError(f"Unknown party: {party_id}")
        version = relation_version or case.get("current_relation_version")
        if version is None:
            raise ValueError("A stance must reference an existing RelationVersion")
        path = self.root / "shared" / "relation" / "versions" / f"v{int(version):04d}.json"
        relation = read_json(path)
        return self.append_event(
            event_type="STANCE_RECORDED", actor=actor, authority_ref=authority_ref,
            relation_version=int(version),
            payload={
                "party_id": party_id, "stance": stance,
                "relation_content_hash": relation.get("content_hash"),
                "notes": notes,
            },
        )

    def attach_evidence(
        self, *, source: Path, evidence_type: str, actor: str,
        relation_version: int | None = None, description: str | None = None,
    ) -> Path:
        if not source.exists() or not source.is_file():
            raise FileNotFoundError(source)
        data = source.read_bytes()
        digest = hashlib.sha256(data).hexdigest()
        destination = self.root / "shared" / "evidence" / f"{digest[:16]}-{source.name}"
        destination.parent.mkdir(parents=True, exist_ok=True)
        if not destination.exists():
            shutil.copy2(source, destination)
        self.append_event(
            event_type="EVIDENCE_ATTACHED", actor=actor,
            relation_version=relation_version or self.case().get("current_relation_version"),
            payload={
                "evidence_type": evidence_type, "filename": destination.name,
                "sha256": digest, "description": description,
            },
        )
        return destination

    def record_scoped_reopen(
        self, *, trigger: str, affected_nodes: list[str], actor: str,
        source_event_ref: str | None = None,
    ) -> dict[str, Any]:
        return self.append_event(
            event_type="SCOPED_REOPEN", actor=actor,
            relation_version=self.case().get("current_relation_version"),
            payload={
                "trigger": trigger, "affected_nodes": sorted(set(affected_nodes)),
